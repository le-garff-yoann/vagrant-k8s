...
stagingdir=$(cat $conffile | yq -cr .srvkube.host) || exit 1
...
virtual_domain=$(cat $conffile | yq -cr .virtual.domain)
...
DNS.5 = kubernetes.default.svc.$(cat $conffile | yq -cr .k8s.domain)
DNS.6 = k8s-api
DNS.7 = k8s-api.$virtual_domain
IP.1 = 127.0.0.1
IP.2 = $nodes_network.1
IP.3 = $nodes_vip
IP.4 = $(cat $conffile | yq -cr .k8s.services.kubeapi.ip)
EOF
...
do
    ...
    ip=$nodes_network.$(($nodes_ip_startafter+$i))
